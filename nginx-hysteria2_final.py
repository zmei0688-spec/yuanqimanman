...
""")
        ...
        # Hysteria2官方链接格式（简化）
        config_link = f"hysteria2://{urllib.parse.quote(password)}@{server_address}:{port}?insecure={insecure_param}&sni={sni_name}&alpn={alpn_value}&up={bw_up}&down={bw_down}"
...
def show_final_summary(server_address, port, port_range, password, obfs_password, config_link, enable_port_hopping=False, download_links=None, num_ports=None, sni_name=DEFAULT_SNI, alpn_value=DEFAULT_ALPN, bw_up=DEFAULT_BW_UP, bw_down=DEFAULT_BW_DOWN):
    ...
    hysteria_443_url = f"hysteria2://{urllib.parse.quote(password)}@{server_address}:{port_range if port_range else 443}?insecure=1&sni={sni_name}&alpn={alpn_value}&up={bw_up}&down={bw_down}&obfs=salamander&obfs-password={urllib.parse.quote(obfs_password)}#Hysteria2-AutoHop"
    ...
    if port_range and '-' in str(port_range):
        ...
        for i, random_port in enumerate(random_ports, 1):
            random_url = f"hysteria2://{urllib.parse.quote(password)}@{server_address}:{port_range if port_range else random_port}?insecure=1&sni={sni_name}&alpn={alpn_value}&up={bw_up}&down={bw_down}&obfs=salamander&obfs-password={urllib.parse.quote(obfs_password)}#HY2-AutoHop-{i:02d}"
