...
DEFAULT_SNI = "www.nvidia.com"
DEFAULT_ALPN = "h3"
DEFAULT_BW_UP = 30
DEFAULT_BW_DOWN = 100

def parse_alpn_list(alpn_value):
    """将逗号分隔的ALPN字符串转换为列表"""
    if isinstance(alpn_value, list):
        return [str(x).strip() for x in alpn_value if str(x).strip()]
    return [item.strip() for item in str(alpn_value).split(",") if item.strip()] or [DEFAULT_ALPN]

def build_hy2_link(server_address, port_value, password, obfs_password, sni_name, alpn_value, bw_up, bw_down, insecure=1, tag="Hysteria2"):
    """构建Hysteria2分享链接"""
    return (
        f"hysteria2://{urllib.parse.quote(password)}@{server_address}:{port_value}"
        f"?insecure={insecure}&sni={sni_name}"
        f"&alpn={urllib.parse.quote(str(alpn_value))}"
        f"&up={bw_up}&down={bw_down}"
        f"&obfs=salamander&obfs-password={urllib.parse.quote(obfs_password or '')}"
        f"#{tag}"
    )
...
def create_config(base_dir, port, password, cert_path, key_path, domain, enable_web_masquerade=True, custom_web_dir=None, enable_port_hopping=False, obfs_password=None, enable_http3_masquerade=False, sni_name=DEFAULT_SNI, alpn_value=DEFAULT_ALPN, bw_up=DEFAULT_BW_UP, bw_down=DEFAULT_BW_DOWN):
    ...
    config = {
        ...
        "tls": {
            "cert": cert_path,
            "key": key_path,
            "alpn": parse_alpn_list(alpn_value)
        },
        "auth": {
            "type": "password",
            "password": password
...
def show_client_setup(config_link, server_address, port, password, use_real_cert, enable_port_hopping=False, obfs_password=None, enable_http3_masquerade=False, sni_name=DEFAULT_SNI, alpn_value=DEFAULT_ALPN, bw_up=DEFAULT_BW_UP, bw_down=DEFAULT_BW_DOWN):
    """显示客户端连接指南"""
    # 构建端口范围
    port_range = None
    if enable_port_hopping:
...
def main():
    ...
    parser.add_argument('--sni', default=DEFAULT_SNI,
                      help='指定客户端SNI域名 (默认: www.nvidia.com)')
    parser.add_argument('--alpn', default=DEFAULT_ALPN,
                      help='指定ALPN (默认: h3)')
    parser.add_argument('--bw-up', type=int, default=DEFAULT_BW_UP,
                      help='客户端/服务端上行带宽 Mbps (默认: 30)')
    parser.add_argument('--bw-down', type=int, default=DEFAULT_BW_DOWN,
                      help='客户端/服务端下行带宽 Mbps (默认: 100)')
    
    
    args = parser.parse_args()
    sni_name = args.sni or DEFAULT_SNI
    alpn_value = args.alpn or DEFAULT_ALPN
    bw_up = args.bw_up or DEFAULT_BW_UP
    bw_down = args.bw_down or DEFAULT_BW_DOWN
...
""")
        ...
        port_range_value = None
        try:
            global_config_path = f"{base_dir}/global_config.json"
            if os.path.exists(global_config_path):
                with open(global_config_path, 'r') as gf:
                    global_cfg = json.load(gf)
                port_range_value = global_cfg.get("port_range")
        except Exception:
            port_range_value = None

        config_link = build_hy2_link(
            server_address,
            port_range_value if port_range_value else port,
            password,
            args.obfs_password or "",
            sni_name,
            alpn_value,
            bw_up,
            bw_down,
            insecure=insecure_param,
            tag="Hysteria2-AutoHop" if port_range_value else "Hysteria2"
        )
        
        show_client_setup(config_link, server_address, port, password, use_real_cert, bool(port_range_value), args.obfs_password, args.http3_masquerade, sni_name, alpn_value, bw_up, bw_down)
...
""")
        ...
        config_path = create_config(base_dir, port, password, cert_path, key_path, 
                                  server_address, args.web_masquerade, web_dir, args.port_hopping, args.obfs_password, args.http3_masquerade, sni_name, alpn_value, bw_up, bw_down)
        ...
        # 构建链接参数
        params = [f"insecure={insecure_param}", f"sni={sni_name}", f"alpn={alpn_value}", f"up={bw_up}", f"down={bw_down}"]
        ...
        config_link = build_hy2_link(server_address, (f"{port_start}-{port_end}" if args.port_hopping else port), password, args.obfs_password or "", sni_name, alpn_value, bw_up, bw_down, insecure=insecure_param, tag="Hysteria2-AutoHop" if args.port_hopping else "Hysteria2")
...
""")
        ...
        # 显示客户端连接指南
        show_client_setup(config_link, server_address, port, password, use_real_cert, args.port_hopping, args.obfs_password, args.http3_masquerade, sni_name, alpn_value, bw_up, bw_down)
    else:
...
def deploy_hysteria2_complete(server_address, port=443, password="123qwe!@#QWE", enable_real_cert=False, domain=None, email="admin@example.com", port_range=None, enable_bbr=False, sni_name=DEFAULT_SNI, alpn_value=DEFAULT_ALPN, bw_up=DEFAULT_BW_UP, bw_down=DEFAULT_BW_DOWN):
    """
    ...
    # 生成分享链接：启用端口跳跃时优先输出端口范围链接
    primary_port_value = f"{port_start}-{port_end}" if port_range else port
    config_link = build_hy2_link(server_address, primary_port_value, password, obfs_password, sni_name, alpn_value, bw_up, bw_down, insecure=insecure, tag="Hysteria2-AutoHop" if port_range else "Hysteria2")
...
def show_final_summary(server_address, port, port_range, password, obfs_password, config_link, enable_port_hopping=False, download_links=None, num_ports=None, sni_name=DEFAULT_SNI, alpn_value=DEFAULT_ALPN, bw_up=DEFAULT_BW_UP, bw_down=DEFAULT_BW_DOWN):
    import urllib.parse
    ...
    print(f"   • ALPN: {alpn_value}")
    print(f"   • 带宽: Up {bw_up} / Down {bw_down} Mbps")
    if obfs_password:
    ...
    if download_links:
        ...
        print("   🔹 官方客户端用户:")
        print("     - 使用官方客户端配置")
        print(f"   🔹 多端口说明: 已改为输出自动跳跃链接，客户端可直接使用端口范围")
    ...
    # 自动跳跃链接 + 443固定链接
    print(f"\n\033[93m🔀 自动跳跃连接地址:\033[0m")
    auto_hop_url = build_hy2_link(
        server_address,
        port_range if port_range else port,
        password,
        obfs_password,
        sni_name,
        alpn_value,
        ...
        bw_down,
        insecure=1,
        tag="Hysteria2-AutoHop"
    )
    print(f"   {auto_hop_url}")

    print(f"\n\033[93m🎯 443端口连接地址:\033[0m")
    hysteria_443_url = build_hy2_link(
        server_address,
        443,
        password,
        obfs_password,
